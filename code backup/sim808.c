 ...
 */
	...
	sim->AT_uart->Instance = USART2;
  sim->AT_uart->Init.BaudRate = 9600;
  sim->AT_uart->Init.WordLength = UART_WORDLENGTH_8B;
  sim->AT_uart->Init.StopBits = UART_STOPBITS_1;
  sim->AT_uart->Init.Parity = UART_PARITY_NONE;
  sim->AT_uart->Init.Mode = UART_MODE_TX_RX;
  sim->AT_uart->Init.HwFlowCtl = UART_HWCONTROL_NONE;
  sim->AT_uart->Init.OverSampling = UART_OVERSAMPLING_16;
  sim->AT_uart->Init.OneBitSampling = UART_ONE_BIT_SAMPLE_DISABLE;
  sim->AT_uart->AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_NO_INIT;
	if (HAL_UART_Init(sim->AT_uart) != HAL_OK) return 1;
	
	sim->debug_uart->Instance = USART1;
  sim->debug_uart->Init.BaudRate = 9600;
  sim->debug_uart->Init.WordLength = UART_WORDLENGTH_8B;
  sim->debug_uart->Init.StopBits = UART_STOPBITS_1;
  sim->debug_uart->Init.Parity = UART_PARITY_NONE;
  sim->debug_uart->Init.Mode = UART_MODE_TX_RX;
  sim->debug_uart->Init.HwFlowCtl = UART_HWCONTROL_NONE;
  sim->debug_uart->Init.OverSampling = UART_OVERSAMPLING_16;
  sim->debug_uart->Init.OneBitSampling = UART_ONE_BIT_SAMPLE_DISABLE;
  sim->debug_uart->AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_NO_INIT;
	if (HAL_UART_Init(sim->debug_uart) != HAL_OK) return 1;
